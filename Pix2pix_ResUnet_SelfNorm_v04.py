...
tf.config.set_visible_devices(gpus[4:5], 'GPU')
if gpus:
...
from utils import resize, load_files_from_directory, load_h5,normalize_tensor
from models import Generator, Discriminator
from evaluation import evaluate_model
from losses import generator_loss, discriminator_loss
from mainargs import get_args
from ResidualUnet import Res_Unet_Generator

############################## Basic configurations ##################################
dataset= "geomcorrected"
mode='triple'  #2.5 D or single (2D)
data_version='_v3.3'
experiment = "/experiments/Pix2Pix_ResUNet_"+dataset+"_"+mode+data_version+"_"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"/"  # 
...
val_dirNorm=os.path.join(PATH, 'target','val')
val_dirUnnorm=os.path.join(PATH,'input',dataset, mode, 'val')

test_dirNorm=os.path.join(PATH, 'target','test')
test_dirUnnorm=os.path.join(PATH,'input',dataset, mode, 'test')
...
with tf.device('/device:cpu:0'):
    ...
    print("\n * Trainset: DONE * \n")
    input_image_val,real_image_val=load_files_from_directory(val_dirUnnorm,val_dirNorm,IMG_HEIGHT, IMG_WIDTH)
    print("\n * Validationset: DONE * \n")
...
print("Total number of training samples ", len(input_image))
print("Total number of validation samples ",len(input_image_val))

input_image.extend(input_image_val)
real_image.extend(real_image_val)

print("Total number of training samples after pooling ", len(input_image))
print("Total number of test samples ",len(input_image_test))
...
for file_name in (os.listdir(test_dirUnnorm)):
    ...
    np.save(outpath+ file_name[:-3]+'_real_A.npy', real_A)
    np.save(outpath+ file_name[:-3]+'_real_B.npy', real_B)
    np.save(outpath+ file_name[:-3]+'_fake_B.npy', fake_B)
    print('file saved for:', file_name)
    images_saved=images_saved+1

print(f"total numer of files saved: {images_saved}k")
...
######################################### Trainset
#Qreport_train=evaluate_model(generator,train_dataset, experiment_dir)

#snr_cum_train=Qreport_train['SNRpr']### predicted vs high dose PET
#ssi_cum_train=np.array(Qreport_train['SSIpr'])
#mse_cum_train=np.array(Qreport_train['MSEpr'])


########## Mean values of the PSNR, SSIM, and MSE #########
#snr_resPR_mean_train = np.mean(snr_cum_train)
#ssim_resPR_mean_train = np.mean(ssi_cum_train)
#mse_resPR_mean_train = np.mean(mse_cum_train)


########## Standard deviation values of the PSNR, SSIM, MSE#########
#snr_resPR_stdTR = np.std(snr_cum_train)
#ssim_resPR_stdTR = np.std(ssi_cum_train)
#mse_resPR_stdTR = np.std(mse_cum_train)

#print("\n * Evalution on the Trainset... * \n")

#print("Mean of the PSNR is ", snr_resPR_mean_train, "+/-", snr_resPR_stdTR) 
#print("Mean of the SSIm is ", ssim_resPR_mean_train,"+/-",ssim_resPR_stdTR) 
#print("Mean of the MSE is",mse_resPR_mean_train  ,"+/-",mse_resPR_stdTR)
