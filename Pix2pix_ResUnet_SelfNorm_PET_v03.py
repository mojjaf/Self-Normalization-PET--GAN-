...
from utils import resize, load_image_data_from_directory, load_h5,normalize_tensor
from models import Generator_ResUnet, Discriminator
...
dataset= "uncorrected_byPatient_3to1_h5"
experiment = "/experiments/Pix2Pix_ResUnet_"+dataset+"_"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"/"  # 
model_name = "model_2D_"+dataset+"_"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"/" 
...
output_preds_2D = experiment_dir+'pix2pix_ResUnet_2.5D_predictions/'
...
train_dirNorm=os.path.join(PATH, dataset, 'A','train')
train_dirUnnorm=os.path.join(PATH,dataset, 'B','train')
val_dirNorm=os.path.join(PATH,dataset, 'A','val')
val_dirUnnorm=os.path.join(PATH,dataset, 'B','val')
test_dirNorm=os.path.join(PATH,dataset, 'A','test')
test_dirUnnorm=os.path.join(PATH, dataset,'B','test')

start_time = time.time()
print("\n * Loading data... (this might take a few minutes) * \n")

with tf.device('/device:cpu:0'):
    input_image,real_image=load_image_data_from_directory(train_dirUnnorm,train_dirNorm,IMG_HEIGHT, IMG_WIDTH)
    print("\n * Trainset: DONE * \n")
    input_image_val,real_image_val=load_image_data_from_directory(val_dirUnnorm,val_dirNorm,IMG_HEIGHT, IMG_WIDTH)
    print("\n * Validationset: DONE * \n")
    input_image_test,real_image_test=load_image_data_from_directory(test_dirUnnorm,test_dirNorm,IMG_HEIGHT, IMG_WIDTH)
    print("\n * Testset:DONE * \n")
...
fit(train_dataset, val_dataset, steps=step_size)  #100000 or 52 epochs
...
for file_nameA,file_nameB in zip(os.listdir(test_dirUnnorm),os.listdir(test_dirNorm)):
    #print(file_name)
    real_A= load_h5(os.path.join(test_dirUnnorm, file_nameA))
    real_B= load_h5(os.path.join(test_dirNorm, file_nameB))
    ...
    np.save(outpath+ file_nameB[:-4]+'_real_A.npy', real_A)
    np.save(outpath+ file_nameA[:-4]+'_real_B.npy', real_B)
    np.save(outpath+ file_nameA[:-4]+'_fake_B.npy', fake_B)
